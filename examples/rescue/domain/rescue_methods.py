...
def tm3_support_person(state, r, p):
    if state.status[state.loc[p]] == "has_debri":
        return [
            ("a_clear_location", r, state.loc[p]),
        ]
    return None
...
def tm4_support_person(state, r, p):
    if state.status[state.loc[p]] == "has_debri":
        return [
            ("a_clear_location_2", r, state.loc[p]),
        ]
    return None
...
def tm5_support_person(state, r, p):
    return [
        ("a_check_real", state.loc[p]),
    ]


# Extra task added.
methods.declare_task_methods(
    "support_task", [tm1_support_person, tm2_support_person, tm3_support_person, tm4_support_person, tm5_support_person]
)
...
def tm1_help_person(state, r, p):
    return [
        ("move_task", r, state.loc[p]),
        ("a_inspect_location", r, state.loc[p]),
        ("a_inspect_person", r, p),
        ("support_task", r, p),
    ]
...
def tm1_survey(state, r, l):
    ...
    return [
        ("adjust_altitute_task", r),
        ("a_capture_image", r, "front_camera", l),
        ("rescue_encap_task", r),
        ("a_check_real", l),
    ]
...
def tm2_survey(state, r, l):
    ...
    return [
        ("adjust_altitute_task", r),
        ("a_capture_image", r, "bottom_camera", l),
        ("rescue_encap_task", r),
        ("a_check_real", l),
    ]
