...
from ipyhop.temporal.utils import parse_iso8601_datetime, format_iso8601_datetime
...
def state_variables(draw):
    ...
    for _ in range(n_vars):
        ...
        if value_type == 'int':
            ...
            vars_dict[name] = {
                'key1': draw(st.integers(0, 100)),
                'key2': draw(st.text(min_size=0, max_size=10))
            }
    return vars_dict
...
def test_state_workflow_simulation(name):
    ...
    state.add_to_timeline(('move', 'home', 'store'), 
                         "2025-01-01T08:00:00Z", "2025-01-01T08:10:00Z")
    
    # Action 2: Buy item
    state.inventory.append('item1')
    state.add_to_timeline(('buy', 'item1'),
                         "2025-01-01T08:10:00Z", "2025-01-01T08:12:00Z")
    ...
    state_copy.add_to_timeline(('move', 'store', 'home'),
                              "2025-01-01T08:12:00Z", "2025-01-01T08:22:00Z")
