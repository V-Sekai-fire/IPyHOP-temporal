...
def task_names(draw):
    ...
    return draw(
        st.text(alphabet=st.characters(whitelist_categories=("Ll", "Lu", "Nd", "_")), min_size=1, max_size=20).filter(
...
def multigoal_tags(draw):
    ...
    if draw(st.booleans()):
        return draw(
            st.text(
                alphabet=st.characters(whitelist_categories=("Ll", "Lu", "Nd", "_")), min_size=1, max_size=20
...
@given(st.text(), method_functions())
...
def test_declare_task_methods_requires_string_task_name(non_string, method_list):
    ...
    assume(not isinstance(non_string, str))
    methods = Methods()
    with pytest.raises(AssertionError):
        methods.declare_task_methods(non_string, method_list)


@given(task_names(), st.lists(st.integers()))
@settings(max_examples=50)
def test_declare_task_methods_requires_list(non_list):
    """declare_task_methods should require list of methods."""
    methods = Methods()
    with pytest.raises(AssertionError):
        methods.declare_task_methods("task", non_list)
...
@given(goal_names(), st.lists(st.integers()))
@settings(max_examples=50)
def test_declare_goal_methods_requires_list(goal_name, non_list):
    """declare_goal_methods should require list of methods."""
    methods = Methods()
    with pytest.raises(AssertionError):
        methods.declare_goal_methods(goal_name, non_list)
...
@given(multigoal_tags(), st.lists(st.integers()))
@settings(max_examples=50)
def test_declare_multigoal_methods_requires_list(multigoal_tag, non_list):
    """declare_multigoal_methods should require list of methods."""
    methods = Methods()
    with pytest.raises(AssertionError):
        methods.declare_multigoal_methods(multigoal_tag, non_list)
